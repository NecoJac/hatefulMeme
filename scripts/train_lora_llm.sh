...
python3 -u -m semantic_rahmd.train \
  ...
  --output-dir "${OUTPUT_DIR:-outputs/lora_qwen_embedding}" \
  ...
  --hf-device-map "${HF_DEVICE_MAP:-auto}" \
  --hf-torch-dtype "${HF_TORCH_DTYPE:-auto}" \
  --epochs "${EPOCHS:-30}" \
